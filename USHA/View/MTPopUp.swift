...
//MARK: - Variable
    ...
    // color
    var headerColor:UIColor = .gray{   // in range of 0.0 to 1.0
        didSet{
            updateColor()
        }
    }
...
//    private var _viewManualHide : Bool
    
    var _viewManualHide: Bool = false
    var viewManualHide: Bool {
        get { return _viewManualHide }
        set {
            _viewManualHide = newValue
            updateGesture()
        }
    }
...
//MARK: - Life Cycle
    override init(frame: CGRect)
    {
        ...
        //here gesture code we move in update gesture.
        updateGesture()
...
//        self.addGestureRecognizer(panGesture)
        ...
        viewHeaderPopup.backgroundColor = UIColor.init(red: 173/255, green: 29/255, blue: 39/255, alpha: 1)//UIColor(red: 37.0/255.0, green: 188.0/255.0, blue: 126.0/255.0, alpha: 1)
        ...
        lblHeader.font = UIFont(name: FontName.HelveticaNeueBold, size: 18)
    ...
    func updateGesture()
    {
        ...
        {
            self.removeGestureRecognizer(gestureDown)
            gestureDown = nil
            self.removeGestureRecognizer(gestureLeft)
            gestureLeft = nil
            self.removeGestureRecognizer(gestureRight)
            gestureRight = nil
            self.removeGestureRecognizer(gestureUp)
            gestureUp = nil
        }
    }
    ...
    func updateColor()
    {
        btnOk.backgroundColor = headerColor;
        viewHeaderPopup.backgroundColor = headerColor;
    }
    
//MARK: - Show Popup
    func show(view: UIView, animationType: NSString, strMessage: String,btnArray: NSArray, strTitle: String = "", themeColor: UIColor, buttonTextColor: UIColor, titleTextColor: UIColor, msgTextColor: UIColor, textAlignment: NSTextAlignment, complete : ((Int)->())?)
    {
        viewHeaderPopup.backgroundColor = themeColor;
        ...
        for strBtnTitle in btnArray
        {
            ...
            let rectNew:CGRect = CGRect(x: ((btnArray.count == 1  && counter == 1) ? rectOld.origin.x : ((btnArray.count == 2 && counter == 2 && isFrameChange == false) ? (viewPopUp.frame.size.width / 2) + 15 : rectOld.origin.x)) ,y: rectOld.origin.y ,width: widthLabel,height: lblButtonNameDummy.requiredHeight() + 20)
    
            btnOk = UIButton(frame: rectNew)
            btnOk.backgroundColor = themeColor
            // UIColor.init(red: 173/255, green: 29/255, blue: 39/255, alpha: 1)//UIColor(red: 37.0/255.0, green: 188.0/255.0, blue: 126.0/255.0, alpha: 1)
            //btnOk.setTitleColor(.black, for: .normal)
            //btnOk.titleLabel?.textColor = buttonTextColor
            //UIColor(red: 223.0/255.0, green: 254.0/255.0, blue: 244.0/255.0, alpha: 1)
            btnOk.layer.borderWidth = 0
            btnOk.layer.borderColor = UIColor.black.cgColor
            btnOk.layer.cornerRadius = 4
            btnOk.titleLabel?.font = UIFont(name: FontName.HelveticaNeue, size: 18)
            ...
            btnOk.setTitleColor(buttonTextColor, for: .normal)
            btnOk.tag = counter
        ...
        lblHeader.textColor = titleTextColor
        viewHeaderPopup.frame = CGRect(x: viewHeaderPopup.frame.origin.x ,y: viewHeaderPopup.frame.origin.y , width: viewHeaderPopup.frame.size.width , height: (lblHeader.requiredHeight() > 1 ? lblHeader.requiredHeight() + 25 : 0))
        ...
        lblMessage.textAlignment = textAlignment
        lblMessage.textColor = msgTextColor
        ...
        let sHeight : CGFloat = screenHeight
        let y = popUpHeight < sHeight ? (((self.screenHeight/2) - ((lblMessage.frame.origin.y + lblMessage.frame.size.height + viewFooterPopup.frame.size.height) / 2)) - 20) : 10
        
        viewPopUp.frame = CGRect(x: viewPopUp.frame.origin.x ,y: y  , width: viewPopUp.frame.size.width , height: popUpHeight)
...
//            })
//        }
        else if animation.isEqual(MTAnimation.ExpandViewAnimation as String)
        {
            let rectOldFrame:CGRect = viewPopUp.frame
            viewPopUp.frame = CGRect(x: viewPopUp.frame.origin.x,y: viewPopUp.frame.origin.y,width: viewPopUp.frame.size.width,height: 0)
            
            UIView.animate(withDuration: 1.0 , delay: 0.0, usingSpringWithDamping: 0.4, initialSpringVelocity: 1.0, options: .curveLinear, animations: {
